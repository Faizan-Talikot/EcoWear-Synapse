...
const DressesPage = () => {
  ...
  const renderItem = ({ item }) => (
    <View style={styles.card}>
      ...
      <TouchableOpacity
        style={styles.button}
        onPress={() => navigation.navigate('ItemDetail', { itemId: item.id, category: item.category })} // Fixed: product -> item
      >
  ...
  return (
    <View style={styles.container}>
      <Text style={styles.header}>Our Dresses Collection</Text>
      <FlatList
        data={dresses}
        keyExtractor={(item) => `dress-${item.id}`}
